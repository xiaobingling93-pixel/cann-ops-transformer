...
/*!
 * \file prompt_flash_attention_pioneer_tiling_v2.cpp
 * \brief
 */
#include "prompt_flash_attention_pioneer_tiling_v2.h"
#include <queue>
...
#include "../../op_kernel/prompt_flash_attention_pioneer/arch35/prompt_flash_attention_pioneer_template_tiling_key.h"
#include "../../op_kernel/prompt_flash_attention_pioneer/arch35/prompt_flash_attention_pioneer_tiling_regbase.h"
...
static uint64_t BENCHMARK_TILING_KEY_ = 1000000000000000000;
...
bool PromptFlashAttentionPioneerTilingV2::CheckNonEmptyShapeExceptions(const ContextParamsForPFATiling& contextKeyParams,
...
void PromptFlashAttentionPioneerTilingV2::PromptFlashAttentionPioneerInitOutputSplit(int64_t totalSize,
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::CheckEmptyTensor(ContextParamsForPFATiling& contextKeyParams) 
{
    ...
    if (enableTensorList) {
        ...
        emptyTensor = ((contextKeyParams.keyInputShape->GetShape().GetShapeSize() == 0) &&
        (contextKeyParams.valueInputShape->GetShape().GetShapeSize() == 0)) ||
        (contextKeyParams.emptyTensor == 1U);
    }
    return ge::GRAPH_SUCCESS;
}

void PromptFlashAttentionPioneerTilingV2::SetEmptyTensor(ContextParamsForPFATiling& contextKeyParams,
    uint32_t& numBlocksToBeSet, PromptFlashAttentionPioneerTilingData& tilingData) 
{
    faRunFlag_ = true;
    quantMode = NoQuantMode;
    PromptFlashAttentionPioneerInitOutputSplit(contextKeyParams.outputShape->GetStorageShape().GetShapeSize(), tilingData);
    tilingData.promptAttentionInitOutputParams.set_totalSoftMaxLseOutputSize(0);
    if (contextKeyParams.isSoftMaxLseEnable) {
        PromptFlashAttentionPioneerInitSoftmaxLseOutputSplit(contextKeyParams.lseoutputShape->GetStorageShape().GetShapeSize(),
            tilingData);
    }
...
bool PromptFlashAttentionPioneerTilingV2::CheckIODataType(ContextParamsForPFATiling& contextKeyParams) 
{
...
bool PromptFlashAttentionPioneerTilingV2::SetInputLayout(const char* layout) 
{
    if (layout == nullptr) {
...
static uint32_t GetTransposeLayoutPionner(const std::string &layout)
{
...
static int64_t GetMaxSeq(const gert::Tensor* actualSeqLength)
{
...
bool PromptFlashAttentionPioneerTilingV2::SetShape(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape* shape,
    const std::string inputName, int64_t& b, int64_t& n, int64_t& s, int64_t& d, int64_t& h, int64_t& t) const 
{
    ...
    if (enablePA && (inputName != "query")) {
        if (shape->GetShape().GetDimNum() == 3) { // 3 for dim num  BSH
            b = shape->GetShape().GetDim(0);
            s = shape->GetShape().GetDim(1);
            h = shape->GetShape().GetDim(2); // 2 for H dim
            n = static_cast<int64_t>(*contextKeyParams.numKeyValueHeads);
            n = n > 0 ? n : static_cast<int64_t>(*contextKeyParams.headsNumber);
            d = n > 0 ? h / n : 0;
        } else if (shape->GetShape().GetDimNum() == 4) { // 4 for dim num
            b = shape->GetShape().GetDim(0);
            n = shape->GetShape().GetDim(1);
            s = shape->GetShape().GetDim(2); // 2 for Sequence length
            d = shape->GetShape().GetDim(3); // 3 for D dim
            h = n * d;
        } else {
            b = shape->GetShape().GetDim(0);
            n = shape->GetShape().GetDim(1);
            s = shape->GetShape().GetDim(3); // 3 for Sequence length
            d = shape->GetShape().GetDim(2) * shape->GetShape().GetDim(4); // 2 for D1 dim，4 for D0 dim
            h = n * d;
        }
...
bool PromptFlashAttentionPioneerTilingV2::GetAndCheckShape(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::GetAndCheckRopeShape(ContextParamsForPFATiling& contextKeyParams, PFAShapeInfo& shapeInfo,
...
void PromptFlashAttentionPioneerTilingV2::GetQueryDimAndOutDim(const gert::StorageShape* queryShape, const gert::StorageShape* outShape,
...
bool PromptFlashAttentionPioneerTilingV2::CheckQueryOutParamsConsistency(const ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckKVDataType(ContextParamsForPFATiling& contextKeyParams) const
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckRopeDataType(ContextParamsForPFATiling& contextKeyParams) const
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckKeyValueParamsConsistency(ContextParamsForPFATiling& contextKeyParams,
    const gert::StorageShape* keyShape, const gert::StorageShape* valueShape) 
{
    if (enableTensorList) {
        return true;
    }
    const size_t keyDimNum = keyShape->GetShape().GetDimNum();
    const size_t valueDimNum = valueShape->GetShape().GetDimNum();
    ...
    for (uint32_t i = 0; i < keyDimNum; ++i) {
        ...
        int64_t tmpKeyDim = keyShape->GetShape().GetDim(i);
        int64_t tmpValueDim = valueShape->GetShape().GetDim(i);
...
bool PromptFlashAttentionPioneerTilingV2::CheckInputDimAndHeadNum(ContextParamsForPFATiling& contextKeyParams,
    const uint32_t nQAttr, const uint32_t nKVAttr) 
{
    ...
    const size_t keyDim = keyShape->GetShape().GetDimNum();
    const size_t valueDim = valueShape->GetShape().GetDimNum();
    ...
    if (((inputLayout == InputLayout::BNSD) || (inputLayout == InputLayout::BSND)) && (!enablePA)) {
        if ((queryDim == 4) && (keyDim == 4) && (valueDim == 4)) { // dim num: 4
            ...
            keyShapeHeadNum = keyShape->GetShape().GetDim(nIdx);
            valueShapeHeadNum = valueShape->GetShape().GetDim(nIdx);
        } else {
...
bool PromptFlashAttentionPioneerTilingV2::SetAndCheckHeadNumRatio(ContextParamsForPFATiling& contextKeyParams, PromptFlashAttentionPioneerTilingData& tilingData) 
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckPostQuantShape(const ContextParamsForPFATiling &contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckPerTensorQuantParams(const ContextParamsForPFATiling& contextKeyParams,
    ...
    OP_CHECK_IF(std::find(unsupportedLayoutList.begin(), unsupportedLayoutList.end(), layoutStr) != unsupportedLayoutList.end(),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
            "%s is not supported in per-tensor quant scenario.", layoutStr.c_str()), return false);
    OP_CHECK_IF((deqScale1Shape == nullptr) || (quantScale1Shape == nullptr) || (deqScale2Shape == nullptr),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
            "DeqScale1, quantScale1 or deqScale2 is nullptr in per-tensor quant scenario."), return false);
    ...
    OP_CHECK_IF(enableAlibiPse, OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, 
            "AlibiPse is not supported in per-tensor quant scenario."), return false);
    ...
    uint64_t keyShapeD = keyShape->GetShape().GetDim(dIdx);
    uint64_t valueShapeD = valueShape->GetShape().GetDim(dIdx);
    if (inputLayout == InputLayout::BSH) {
...
bool PromptFlashAttentionPioneerTilingV2::CheckPerblockQuantParams(const ContextParamsForPFATiling& contextKeyParams, 
...
bool PromptFlashAttentionPioneerTilingV2::CheckPostQuantParams(const ContextParamsForPFATiling& contextKeyParams, 
...
bool PromptFlashAttentionPioneerTilingV2::CheckAntiquantParamsShape(ContextParamsForPFATiling& contextKeyParams) const
{
    const gert::StorageShape* antiquantScaleShape = contextKeyParams.antiquantScaleShape;
...
bool PromptFlashAttentionPioneerTilingV2::GetAndCheckPrefixShape(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckKeyValuePrefixConsistency(ContextParamsForPFATiling& contextKeyParams,
    const gert::StorageShape* keyShape) 
{
    ...
    size_t KVDim = keyShape->GetShape().GetDimNum();
...
bool PromptFlashAttentionPioneerTilingV2::CheckActSharedPrefix(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckPAKeyValueShape(ContextParamsForPFATiling& contextKeyParams, int64_t& keyDim1,
...
{
    int64_t keyDim2 = keyShape->GetShape().GetDim(KV_CACHE_DIM_1);
    int64_t keyDim3 = keyShape->GetShape().GetDim(KV_CACHE_DIM_2);
    int64_t keyDim4 = 0;
    int64_t keyDim5 = 0;
    int64_t valueDim1 = valueShape->GetShape().GetDim(KV_CACHE_DIM_0);
    int64_t valueDim2 = valueShape->GetShape().GetDim(KV_CACHE_DIM_1);
    int64_t valueDim3 = valueShape->GetShape().GetDim(KV_CACHE_DIM_2);
    int64_t valueDim4 = 0;
    int64_t valueDim5 = 0;
    if (keyDim == KV_CACHE_DIM_NUMS_4) {
        keyDim4 = keyShape->GetShape().GetDim(KV_CACHE_DIM_3);
        valueDim4 = valueShape->GetShape().GetDim(KV_CACHE_DIM_3);
    }
    if (keyDim == KV_CACHE_DIM_NUMS_5) {
        keyDim4 = keyShape->GetShape().GetDim(KV_CACHE_DIM_3);
        valueDim4 = valueShape->GetShape().GetDim(KV_CACHE_DIM_3);
        keyDim5 = keyShape->GetShape().GetDim(KV_CACHE_DIM_4);
        valueDim5 = valueShape->GetShape().GetDim(KV_CACHE_DIM_4);
    }
...
bool PromptFlashAttentionPioneerTilingV2::CheckPACacheShape(ContextParamsForPFATiling& contextKeyParams, const size_t keyDim, PFAShapeInfo& shapeInfo,
    const gert::StorageShape* shape, const int32_t* blockSize, int64_t blockNumValid, int32_t headNumRatio, const std::string& sName) 
{
    std::string layoutStr(contextKeyParams.layout);
    int64_t dim1 = shape->GetShape().GetDim(KV_CACHE_DIM_0);
    int64_t dim2 = shape->GetShape().GetDim(KV_CACHE_DIM_1);
    int64_t dim3 = shape->GetShape().GetDim(KV_CACHE_DIM_2);
    int64_t dim4 = 0;
    int64_t dim5 = 0;
    ...
    if (keyDim == 3) {    // dim num: 3
        ...
        dim4 = shape->GetShape().GetDim(3);  // 3: The third dimension.
        tempN = dim2;
        tempBlockSize = dim3;
        tempD = dim4;
        ...
        dim4 = shape->GetShape().GetDim(KV_CACHE_DIM_3);
        dim5 = shape->GetShape().GetDim(KV_CACHE_DIM_4);
        tempN = dim2;
        tempD1 = dim3;
        tempBlockSize = dim4;
        tempD0 = dim5;
...
bool PromptFlashAttentionPioneerTilingV2::CheckBlockTableShape(ContextParamsForPFATiling& contextKeyParams, PFAShapeInfo& queryShapeInfo,
    ...
    PromptFlashAttentionPioneerTilingData& tilingData) 
{
    ...
    const gert::StorageShape* valueShape = contextKeyParams.valueInputShape;
    const size_t keyDim = keyShape->GetShape().GetDimNum();
    int64_t keyDim1 = keyShape->GetShape().GetDim(KV_CACHE_DIM_0);   
    ...
    if (enableIFAMLA || enablePFARope) {
        ...
        const size_t keyRopeDim = keyRopeShape->GetShape().GetDimNum();
...
bool PromptFlashAttentionPioneerTilingV2::CheckMaskShape(ContextParamsForPFATiling& contextKeyParams, const int32_t* sparseMode,
...
void PromptFlashAttentionPioneerTilingV2::SetSparseModeData(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckMaskShapeCrossSparse(ContextParamsForPFATiling& contextKeyParams,
    PromptFlashAttentionPioneerTilingData& tilingData, const int32_t* sparseMode, uint32_t sQ, const uint32_t sK,
...
bool PromptFlashAttentionPioneerTilingV2::CheckPFAMerge(ContextParamsForPFATiling& contextKeyParams,
    const PFAShapeInfo& queryShapeInfo) const 
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckIO(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckKV(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckQueryAndKey(ContextParamsForPFATiling& contextKeyParams,
    PFAShapeInfo& queryShapeInfo, PFAShapeInfo& keyShapeInfo, PromptFlashAttentionPioneerTilingData& tilingData) 
{
    // check numhead ratio
...
bool PromptFlashAttentionPioneerTilingV2::CheckIFAMLA(ContextParamsForPFATiling& contextKeyParams, const PFAShapeInfo& queryShapeInfo) const 
{
    ...
    if (enableIFAMLAFullQuant) {
        ...
        static const std::set<uint32_t> supportNumHeadInIfaMla = {1U, 2U, 3U, 4U, 6U, 8U, 12U, 16U, 24U, 32U, 48U, 64U, 96U, 128U}; // ifa mla场景qN支持范围
        OP_CHECK_IF((supportNumHeadInIfaMla.find(queryShapeInfo.n) == supportNumHeadInIfaMla.end()),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "input query's heads num is %u, it should be in range of "
                "{1U, 2U, 3U, 4U, 6U, 8U, 12U, 16U, 24U, 32U, 48U, 64U, 96U, 128U} when enable ifa mla", queryShapeInfo.n),
            return false);
    }
...
bool PromptFlashAttentionPioneerTilingV2::CheckRope(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckLayout(ContextParamsForPFATiling& contextKeyParams) const 
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckQuant(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckQScaleShape4MLAFullQuant(ContextParamsForPFATiling& contextKeyParams) const 
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckKVScaleShape4MLAFullQuant(ContextParamsForPFATiling& contextKeyParams) const 
{
    const gert::StorageShape* keyAntiquantScaleShape = contextKeyParams.KeyAntiquantScaleShape;
    const gert::StorageShape* valueAntiquantScaleShape = contextKeyParams.valueAntiquantScaleShape;
...
bool PromptFlashAttentionPioneerTilingV2::CheckMLAFullQuant(ContextParamsForPFATiling& contextKeyParams) const
{
    // check layout
    std::string layoutStr(contextKeyParams.layout);
...
bool PromptFlashAttentionPioneerTilingV2::CheckPrefix(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckActSeq(const ContextParamsForPFATiling& contextKeyParams, const PFAShapeInfo& queryShapeInfo) const 
{
    ...
    if (!enablePA) {
        if (inputLayout == InputLayout::TND) {
            OP_CHECK_IF(actSeqLenKV->GetData<int64_t>()[batchSize - 1] != keyShape->GetShape().GetDim(0),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                "When layout is TND, the last element of Actual_seq_lengths_kv(%ld) must be equal to T(%ld)",
                    actSeqLenKV->GetData<int64_t>()[batchSize - 1], keyShape->GetShape().GetDim(0)),
                return false);
        } else {
            OP_CHECK_IF(actSeqLenKV->GetData<int64_t>()[batchSize - 1] != keyShape->GetShape().GetDim(1),
                OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                "When layout is NTD, the last element of Actual_seq_lengths_kv(%ld) must be equal to T(%ld)",
                    actSeqLenKV->GetData<int64_t>()[batchSize - 1], keyShape->GetShape().GetDim(1)),
                return false);
        }
    }
...
bool PromptFlashAttentionPioneerTilingV2::CheckActSeqLen(ContextParamsForPFATiling& contextKeyParams,
        ...
        for (uint32_t i = LOOP_BEGIN_NUM; i < actSeqLengthKVSize; ++i) {
            ...
            if (!enableTensorList && !enablePA) {
                ...
                if ((inputLayout == InputLayout::BSND) || (inputLayout == InputLayout::BSH)) {
                    actSeqKVMax = contextKeyParams.kTensorList[i]->GetShape().GetDim(1);
                } else {
                    actSeqKVMax = contextKeyParams.kTensorList[i]->GetShape().GetDim(2); // 2: Dim S for BNSD
                }
                OP_CHECK_IF((actSeqKVTmp < 0) || (actSeqKVTmp > actSeqKVMax),
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                        "Actual_seq_lengths_kv[%u](%ld) must be in range[0, %ld]!", i, actSeqKVTmp, actSeqKVMax),
...
bool PromptFlashAttentionPioneerTilingV2::CheckPATypeAndShape(ContextParamsForPFATiling& contextKeyParams,
    PFAShapeInfo& queryShapeInfo, PFAShapeInfo& queryRopeShapeInfo, PromptFlashAttentionPioneerTilingData& tilingData) 
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckPseShiftTypeAndShape(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckInnerPrecise(ContextParamsForPFATiling& contextKeyParams,
    PromptFlashAttentionPioneerTilingData& tilingData) 
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckMaskTypeAndShape(ContextParamsForPFATiling& contextKeyParams,
    PromptFlashAttentionPioneerTilingData& tilingData) 
{
    const gert::StorageShape* attenMaskShape = contextKeyParams.attentionMaskShape;
...
void PromptFlashAttentionPioneerTilingV2::SetSparseType(uint32_t qS)
{
    if (!faRunFlag_) {
        return;
    }
...
bool PromptFlashAttentionPioneerTilingV2::CheckSparseMode(ContextParamsForPFATiling& contextKeyParams, uint32_t qS) 
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckNocontinuesStirde(ContextParamsForPFATiling& contextKeyParams,
    PromptFlashAttentionPioneerTilingData& tilingData) {

    keyNoContinuesStride = contextKeyParams.keyNoContinuesStride;
    keyRopeNoContinuesStride = contextKeyParams.keyRopeNoContinuesStride;

    return true;
}

bool PromptFlashAttentionPioneerTilingV2::CheckPACrossover(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckMaskCrossIFAMLA(ContextParamsForPFATiling& contextKeyParams,
...
bool PromptFlashAttentionPioneerTilingV2::CheckMaskCrossover(ContextParamsForPFATiling& contextKeyParams,
    PFAShapeInfo& queryShapeInfo, PromptFlashAttentionPioneerTilingData& tilingData) 
{
    auto maskDataType = contextKeyParams.maskDataType;
    const int32_t* sparseMode = contextKeyParams.sparseMode;
    const gert::StorageShape* attenMaskShape = contextKeyParams.attentionMaskShape;
...
bool PromptFlashAttentionPioneerTilingV2::CheckTNDLayoutCrossover(ContextParamsForPFATiling& contextKeyParams) 
{
    if (inputLayout != InputLayout::TND) {
        return true;
    }
...
bool PromptFlashAttentionPioneerTilingV2::CheckNTDLayoutCrossover(ContextParamsForPFATiling& contextKeyParams, PFAShapeInfo& queryShapeInfo) 
{
    if (inputLayout != InputLayout::NTD) {
        return true;
    }
...
bool PromptFlashAttentionPioneerTilingV2::CheckTransposeLayoutCrossover(ContextParamsForPFATiling& contextKeyParams,
    PFAShapeInfo& queryShapeInfo) 
{
    std::string layoutStr(contextKeyParams.layout);
...
bool PromptFlashAttentionPioneerTilingV2::CheckLearnSink(ContextParamsForPFATiling &contextKeyParams, PFAShapeInfo &queryShapeInfo)
{
    if (!enableLearnSink) {
        return true;
    }
...
bool PromptFlashAttentionPioneerTilingV2::ParseActualSeqLengths(ContextParamsForPFATiling& contextKeyParams,
    ...
    for (uint32_t i = LOOP_BEGIN_NUM; i < lenDims; i++) {
        ...
        if (!enableActSeqLenKV) {       // The user did not input act_seq_kv
            if (!enableTensorList) {
                ...
                if ((inputLayout == InputLayout::BSND) || (inputLayout == InputLayout::BSH)) {
                    actualSeqLengthsKV[i] = contextKeyParams.kTensorList[i]->GetShape().GetDim(1);
                } else { // 2: Obtain the second dimension
                    actualSeqLengthsKV[i] = contextKeyParams.kTensorList[i]->GetShape().GetDim(2);
                }
            }
        } else {
...
bool PromptFlashAttentionPioneerTilingV2::CheckPerblockCrossover(ContextParamsForPFATiling& contextKeyParams) 
{
    if (!enablePerblockQuant) {
        return true;
    }
...
void PromptFlashAttentionPioneerTilingV2::SetTilingDataAttribute(ContextParamsForPFATiling& contextKeyParams,
    PromptFlashAttentionPioneerTilingData& tilingData) 
{
    ...
    tilingData.promptAttentionBaseParams.set_sinkLength(sinkLength);
    tilingData.promptAttentionBaseParams.set_keyNoContinuesStride(keyNoContinuesStride);
    tilingData.promptAttentionBaseParams.set_keyRopeNoContinuesStride(keyRopeNoContinuesStride);
...
void PromptFlashAttentionPioneerTilingV2::GetEnableDN(ContextParamsForPFATiling& contextKeyParams,
    PromptFlashAttentionPioneerTilingData& tilingData, PFAShapeInfo& queryShapeInfo, PFAShapeInfo& valueShapeInfo,
...
void PromptFlashAttentionPioneerTilingV2::SetTilingData(ContextParamsForPFATiling& contextKeyParams, PFAShapeInfo& queryShapeInfo,
    PFAShapeInfo& queryRopeShapeInfo, PFAShapeInfo& valueShapeInfo, PromptFlashAttentionPioneerTilingData& tilingData) 
{
...
void PromptFlashAttentionPioneerTilingV2::InferTilingMod(const ContextParamsForPFATiling& contextKeyParams,
...
void PromptFlashAttentionPioneerTilingV2::InferSplitCoreMode() 
{
    splitCoreMode = SplitCoreMode::SPLIT_NBS_CUBE;
}

void PromptFlashAttentionPioneerTilingV2::InferConstantization() 
{
    isConstantization = true;
}

void PromptFlashAttentionPioneerTilingV2::GetMatMulType(matmul_tiling::DataType &mmInputType,
...
bool PromptFlashAttentionPioneerTilingV2::EnableMTE2BmmPipe(PromptFlashAttentionPioneerTilingData& tilingData,
    matmul_tiling::MatmulApiTiling& bmm, TCubeTiling& bmmTilingData, uint32_t sOuterFactor, uint32_t sInnerFactor) 
{
    ...
    OP_CHECK_IF(ret != 0, OPS_REPORT_VECTOR_INNER_ERR("PromptFlashAttentionPioneer", "bmm SetFixSplit failed, ret = %d!", ret),
        return false);
    bool res = bmm.GetTiling(bmmTilingData) != -1;
    return res;
}

void PromptFlashAttentionPioneerTilingV2::EnableBmmDoubleBuffer(TCubeTiling& bmmTilingData) const
{
...
bool PromptFlashAttentionPioneerTilingV2::PromptFlashAttentionPioneerCheckBmm1(PromptFlashAttentionPioneerTilingData& tilingData,
...
bool PromptFlashAttentionPioneerTilingV2::AdjustCVTilingCVDiff(const ContextParamsForPFATiling& contextKeyParams,
    uint32_t& sOuterFactor, uint32_t& sInnerFactor, uint32_t& softmaxSOuterFactor,
    PromptFlashAttentionPioneerTilingData& tilingData, const PFAShapeInfo& queryShapeInfo) 
{
    ...
    bool res = PromptFlashAttentionPioneerComputeCVDiffParams(tilingData, compileInfoPtr->l1Size, compileInfoPtr->l0CSize,
        minFactor, rectangleFactor);
    OP_CHECK_IF(res == false, OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
        "PromptFlashAttentionPioneerComputeCVDiffParams failed!"),
        return false);
...
bool PromptFlashAttentionPioneerTilingV2::PromptFlashAttentionPioneerCheckBmm2(PromptFlashAttentionPioneerTilingData& tilingData,
...
bool PromptFlashAttentionPioneerTilingV2::PromptFlashAttentionPioneerComputeCVDiffParams(PromptFlashAttentionPioneerTilingData& tilingData,
    int64_t l1Size, int64_t l0CSize, uint32_t& sOuterFactor, uint32_t &sInnerFactor) 
{
    ...
    bool res = PromptFlashAttentionPioneerCheckBmm1(tilingData, tilingData.bmm1TilingDataRect, l1SizeRemain, l0CSize, sOuterFactor,
        sInnerFactor, true);
    OP_CHECK_IF(!res, OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName, "PromptFlashAttentionPioneerCheckBmm1 failed!"),
        return false);

    res = PromptFlashAttentionPioneerCheckBmm2(tilingData, tilingData.bmm2TilingDataRect, l1SizeRemain, l0CSize, sOuterFactor,
        sInnerFactor, dSplitFactorBmm2, true);
    OP_CHECK_IF(!res, OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName, "PromptFlashAttentionPioneerCheckBmm2 failed!"),
        return false);

    return true;
}

void PromptFlashAttentionPioneerTilingV2::GetPreNextTokensLeftUp(PromptFlashAttentionPioneerTilingData& tilingData,
...
int64_t PromptFlashAttentionPioneerTilingV2::GetActualInnerBlockNums(int64_t sInnerIndexStart, int64_t sInnerIndexEnd,
...
int64_t PromptFlashAttentionPioneerTilingV2::SumOfArithmeticSeries(int64_t an, int64_t d) const 
{
...
int64_t PromptFlashAttentionPioneerTilingV2::SinkBlockCount(int64_t actualSeqLength, int64_t actualSeqLengthKV,
    uint32_t sOuterSize, uint32_t sInnerSize) {
    int64_t SinkBlock = 0;
    //S1方向进行分核，把不同sOuterIndex分给不同核，计算当前batch下的S1方向基本块数量
    int64_t outerBlockNums = (actualSeqLength + sOuterSize - 1) / sOuterSize;
    //求出sink部分有多少个S2基本块
    int64_t sinkBlockNumberPerS1g = (sinkLength + static_cast<int64_t>(sInnerSize) - 1) / static_cast<int64_t>(sInnerSize);
    SinkBlock = sinkBlockNumberPerS1g * outerBlockNums;
    return SinkBlock;
}

int64_t PromptFlashAttentionPioneerTilingV2::GetCutBlockNums(int64_t blockSeqLengthKV, int64_t blockSeqLength,
...
void PromptFlashAttentionPioneerTilingV2::FixParamWithRowInvalid(int64_t& actualSeqLength, int64_t actualSeqLengthKV,
...
int64_t PromptFlashAttentionPioneerTilingV2::ClipSInnerToken(int64_t sInnerToken, int64_t minValue, int64_t maxValue)
{
...
int64_t PromptFlashAttentionPioneerTilingV2::GetCalcBlockNumsForS1G(int64_t actualSeqLength, int64_t actualSeqLengthKV,
    uint32_t sOuterSize, uint32_t sInnerSize, int64_t preTokensLeftUp, int64_t nextTokensLeftUp)
{
    int64_t s2BlockNum = 0;
...
int64_t PromptFlashAttentionPioneerTilingV2::GetCalcBlockNumsOneHead(int64_t actualSeqLength, int64_t actualSeqLengthKV,
    uint32_t sOuterSize, uint32_t sInnerSize, int64_t preTokensLeftUp, int64_t nextTokensLeftUp, bool isAttenMaskUsed)
{
    if (!isAttenMaskUsed) {
        ...
        if (enableIFAMLA) {
            return GetCalcBlockNumsForS1G(actualSeqLength, actualSeqLengthKV, sOuterSize, sInnerSize, preTokensLeftUp, nextTokensLeftUp);
        }
...
void PromptFlashAttentionPioneerTilingV2::ComputeSplitNBSeq(PromptFlashAttentionPioneerTilingData& tilingData, uint32_t batchSize,
    ...
    uint32_t sOuterSize, uint32_t sInnerSize, double coreWightTarget, uint32_t& curCore) 
{
    ...
    int64_t sinkBlockNumberPerS1g = 0;
    if (sinkLength > 0) {
        sinkBlockNumberPerS1g = (sinkLength + static_cast<int64_t>(sInnerSize) - 1) / static_cast<int64_t>(sInnerSize);
    }
        ...
        for (uint32_t headNum = 0; headNum < baseParams->get_headNumSize(); headNum++) {
            ...
            for (uint32_t sOuterIndex = 0; sOuterIndex < outerBlockNums; sOuterIndex++) {
                ...
                if (enableIFAMLA) {
                    ...
                    actualInnerBlockNums = GetActualInnerBlockNums(sInnerIndexStart, sInnerIndexEnd, innerBlockNums) +
...
void PromptFlashAttentionPioneerTilingV2::SetMultiCoreParamsRegbase(int64_t totalSize, int64_t actualUsedCoreNum)
{
...
void PromptFlashAttentionPioneerTilingV2::PromptFlashAttentionPioneerSplitNBSeq(PromptFlashAttentionPioneerTilingData& tilingData,
    std::vector<int64_t>& actualSeqLengths, std::vector<int64_t>& actualSeqLengthsKV, bool isAttenMaskUsed) 
{
    ...
    for (uint32_t sIdx = 0; sIdx < batchSize; sIdx++) {
        ...
        if(sinkLength > 0){
            totalBlockNumsOneHead += SinkBlockCount(actualSeqLengthsTmp, actualSeqLengthsKV[sIdx], sOuterSize,
            sInnerSize);
        }
    }
...
void PromptFlashAttentionPioneerTilingV2::PromptFlashAttentionPioneerInitSoftmaxLseOutputSplit(int64_t totalSize,
    PromptFlashAttentionPioneerTilingData &tilingData) 
{
    PromptAttentionInitOutputParams *initParams = &tilingData.promptAttentionInitOutputParams;
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyLayoutType() 
{
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyConfig(ContextParamsForPFATiling& contextKeyParams, PromptFlashAttentionPioneerTilingData &tilingData) 
{
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyPseMode() 
{
    if (usePseShift == 0U) {
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyQuantMode(ge::DataType inputDataType) 
{
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyAttenMask(ge::DataType inputDataType) 
{
    // perblock采用新模板
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyHasRope(ge::DataType inputDataType) 
{
    if (enablePertensorQuant && (inputDataType == ge::DT_INT8)) {
        hasRope = 0;
        return;
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyIsPa(ge::DataType inputDataType) 
{
    if (enablePertensorQuant && (inputDataType == ge::DT_INT8)) {
        isPa = 0;
        return;
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyIsFd(ge::DataType inputDataType) 
{
    if (isKVHasPrefix) {
        isFd = false;
        return;
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyEmptyTensor() 
{
    emptyTensor = 0;
}

void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyPFAMask(PromptFlashAttentionPioneerTilingData &tilingData, ge::DataType inputDataType) 
{
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyPFAMatMulType(PromptFlashAttentionPioneerTilingData &tilingData, ge::DataType inputDataType) 
{
    if (enablePerblockQuant || enableIFAMLAFullQuant || inputDataType == ge::DT_FLOAT16 || inputDataType == ge::DT_BF16) {
        pFAMatMulType = 0;
        return;
...
void PromptFlashAttentionPioneerTilingV2::UpdateTilingKeyEnableKVPrefix() 
{
    enableKVPrefix = isKVHasPrefix;
}

bool PromptFlashAttentionPioneerTilingV2::TilingGetTilingKeyAttentionAscendC(ContextParamsForPFATiling& contextKeyParams, PromptFlashAttentionPioneerTilingData &tilingData) 
{
...
size_t PromptFlashAttentionPioneerTilingV2::GetPFAWorkSpaceSize(PromptFlashAttentionPioneerTilingData& tilingData) 
{
    auto platformInfoPtr = context_->GetPlatformInfo();
    OP_CHECK_IF(platformInfoPtr == nullptr,
        OPS_REPORT_VECTOR_INNER_ERR(context_->GetNodeName(), "platformInfoPtr is null!"),
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::SetPlatMemoryInfo(ContextParamsForPFATiling& contextKeyParams) 
{
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::SetAttributeInfo(ContextParamsForPFATiling& contextKeyParams) 
{
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::CheckTensorInvalid(const ContextParamsForPFATiling& contextKeyParams) const 
{
...
bool PromptFlashAttentionPioneerTilingV2::CheckAlibiPseShiftTypeAndShape(ContextParamsForPFATiling& contextKeyParams, uint32_t n)
{
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::CheckSingleAttribute(ContextParamsForPFATiling& contextKeyParams, PFAShapeInfo& queryShapeInfo,
    PFAShapeInfo& keyShapeInfo, PFAShapeInfo& valueShapeInfo, PFAShapeInfo& queryRopeShapeInfo, PromptFlashAttentionPioneerTilingData& tilingData) 
{
    ...
    if (!CheckNocontinuesStirde(contextKeyParams, tilingData)) {
        OP_LOGE(contextKeyParams.opName, "CheckNocontinuesStirde failed!");
        return ge::GRAPH_FAILED;
    }
...
bool PromptFlashAttentionPioneerTilingV2::CheckAlibiPseCrossover(ContextParamsForPFATiling& contextKeyParams) 
{
    if (!enableAlibiPse) {
        return true;
    }
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::CheckCrossoverAttribute(ContextParamsForPFATiling& contextKeyParams,
    PFAShapeInfo& queryShapeInfo, PromptFlashAttentionPioneerTilingData& tilingData) 
{
    ...
    if (!CheckSinkLengthCrossover(contextKeyParams, queryShapeInfo)) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::AdjustTilingData(ContextParamsForPFATiling& contextKeyParams,
    PromptFlashAttentionPioneerTilingData& tilingData, const PFAShapeInfo& queryShapeInfo) 
{
...
bool PromptFlashAttentionPioneerTilingV2::IsFlashDecode(ContextParamsForPFATiling& contextKeyParams, uint64_t bng) const
{
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::SplitBNS(PromptFlashAttentionPioneerTilingData& tilingData, uint64_t bng) 
{
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::ComputeTilingData(ContextParamsForPFATiling& contextKeyParams,
    std::vector<int64_t>& actualSeqLengths, std::vector<int64_t>& actualSeqLengthsKV,
    PromptFlashAttentionPioneerTilingData& tilingData) 
{
    // Compute tiling data.
    if (splitCoreMode == SplitCoreMode::SPLIT_NBS_CUBE) {
        ...
        PromptFlashAttentionPioneerSplitNBSeq(tilingData, actualSeqLengths, actualSeqLengthsKV, isAttenMaskUsed);
    }
    ...
    if (enableIFA && !enablePFAMerge) {
        ...
        if (IsFlashDecode(contextKeyParams, bng) && sinkLength == 0) {
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::ComputeTilingKey(ContextParamsForPFATiling& contextKeyParams,
    uint32_t& numBlocksToBeSet, PromptFlashAttentionPioneerTilingData& tilingData) 
{
...
void PromptFlashAttentionPioneerTilingV2::SetAttenMaskCompressMode()
{
...
void PromptFlashAttentionPioneerTilingV2::SetLayoutType()
{
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::SetQKVStartIdx(ContextParamsForPFATiling& contextKeyParams) 
{
    auto &inputParams = faTilingAdapter.inputParamsRegbase;
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::ConvertContextToPFAParams(ContextParamsForPFATiling& contextKeyParams) const
{
    ...
    contextKeyParams.compileInfoPtr = reinterpret_cast<const PromptFlashAttentionPioneerCompileInfo *>(context_->GetCompileInfo());
...
void PromptFlashAttentionPioneerTilingV2::PFATilingDataconvert(PromptFlashAttentionPioneerTilingData& tilingData) 
{
    if (emptyTensor) {
    ...
    inputParams.set_sinkLength(tilingData.promptAttentionBaseParams.get_sinkLength());
    inputParams.set_keyNoContinuesStride(tilingData.promptAttentionBaseParams.get_keyNoContinuesStride());
    inputParams.set_keyRopeNoContinuesStride(tilingData.promptAttentionBaseParams.get_keyRopeNoContinuesStride());
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::PromptFlashAttentionPioneerSetTilingData() const
{
    return ge::GRAPH_SUCCESS;
}

void PromptFlashAttentionPioneerTilingV2::GetMaxWorkspaceFlag(ContextParamsForPFATiling& contextKeyParams) 
{
...
void PromptFlashAttentionPioneerTilingV2::InitializeMaxWorkspace(PFAShapeInfo& queryShapeInfo,
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::RunBigKernelTilingWithParams(ContextParamsForPFATiling& contextKeyParams,
    uint32_t& numBlocksToBeSet, PromptFlashAttentionPioneerTilingData& tilingData) 
{
...
void PromptFlashAttentionPioneerTilingV2::SetTilingKey(ContextParamsForPFATiling& contextKeyParams) const 
{
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::DoSubOpTiling(PromptFlashAttentionPioneerTilingData& tilingData, ContextParamsForPFATiling& contextParamsForPFATiling) {
    ...
    if (faRunFlag_) {
        ...
        *tiling = faTilingAdapter;
        ...
        if (contextParamsForPFATiling.inputDataType != ge::DT_BF16 && contextParamsForPFATiling.inputDataType != ge::DT_FLOAT16) {
            PFAPioneerFullQuantTilingData* tiling = context_->GetTilingData<PFAPioneerFullQuantTilingData>();
            tiling->MigrateFromLegacyFormat(tilingData);
        } else {
            PromptFlashAttentionPioneerTilingData* tiling = context_->GetTilingData<PromptFlashAttentionPioneerTilingData>();
            *tiling = tilingData;
        }
    }
...
ge::graphStatus PromptFlashAttentionPioneerTilingV2::DoOpTiling()
{  
    PromptFlashAttentionPioneerTilingData tilingData;
...
REGISTER_TILING_TEMPLATE_FIA(AttentionPioneer, PromptFlashAttentionPioneerTilingV2, std::vector<int32_t>({static_cast<int32_t>(NpuArch::DAV_3510)}), 90);
} // namespace v2
} // namespace optiling
